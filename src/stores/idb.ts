...
export type IDBStoreCallbacks_t<T> = {
   afterUpdate?: (newState: IDBStoreValue_t<T>, prevState: IDBStoreValue_t<T>) => any | Promise<any>
   onDBCreateSuccess?: (db: IDBDatabase) => void | Promise<void>
   onDBCreateFail?: () => void | Promise<void>
}

export class IDBCollectionStore<T = any> extends BasicStore<IDBStoreValue_t<T>> {
   private _key: string
   ...
   protected _initialValue: IDBStoreValue_t<T> = {}

   constructor(name: string, key: string, version: number, callbacks: IDBStoreCallbacks_t<T>) {
      super()
...
export function makeIDBDatabaseStore<T>(
   ...
   const store = new IDBCollectionStore<T>(config.name, config.key, config.version, callbacks ?? {})
