...
export function onLifecycle(events: {
   ...
   onUpdate?:
      | { callback: () => Promise<void>; dependencies: DependencyList }
      | { callback: () => Promise<void>; dependencies: DependencyList }[]
}): void {
   if (events.onMount) onMount(events.onMount)
   if (events.onUnmount) onUnmount(events.onUnmount)
   if (events.onUpdate) {
      if (Array.isArray(events.onUpdate)) {
         events.onUpdate.forEach((event) => onUpdate(event.callback, event.dependencies))
      } else {
